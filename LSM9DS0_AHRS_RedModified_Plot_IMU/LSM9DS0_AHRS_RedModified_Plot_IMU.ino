...
float mag_offsets[3]            = { 506.13F, -96.26F, -184.13F };

// Soft iron error compensation matrix
float mag_softiron_matrix[3][3] = { {+0.623,+0.138, -0.570 },
                                    {+0.138,+1.597, +0.769 },
                                    {-0.570,+0.769, +2.059 } }; 

float mag_field_strength        = 42.70F;
float mm;
unsigned int tickCount = 0;
long long yawCount = 0;
void loop()
{
  ...
  {  // When new magnetometer data is ready
    ...
    mx = dof.calcMag(dof.mx - mag_offsets[0]);     // Convert to Gauss and correct for calibration
    my = dof.calcMag(dof.my - mag_offsets[1]);
    mz = dof.calcMag(dof.mz - mag_offsets[2]);
