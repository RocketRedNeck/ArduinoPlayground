...
#define SERVO_MESSAGE           0x80 // send data for a servo
...
void setServoPos(byte pin, byte pos);
...
int numServos = 2;
Servo servo;
// starting positions for vertical eye, mouth, eyebrow, ear,
// horizontal eye, and neck
int servoInputs[] = {100, 100};  
...
void printVersion() {
  ...
  Serial.write(FIRMATA_MINOR_VERSION);
  Serial.write(FIRMATA_MAJOR_VERSION);
}
...
void checkDigitalInputs(void) {
  if(digitalInputsEnabled) {
	...
	if(digitalInputs != previousDigitalInputs) {
	  ...
	  Serial.write(digitalInputs % 128); // Tx pins 0-6
	  Serial.write(digitalInputs >> 7);  // Tx pins 7-13
	}
  }
}
...
// -----------------------------------------------------------------------------
/* sets the position of the relevant servo motor
 */
 void setServoPos(byte pin, byte pos) { 
   servo.attach(pin);
   int curPos = servoInputs[pin-3];  // servos are on pins 2 - 7
   int newPos = pos;
   int t = 15;
   if (curPos > newPos)
   {
      for(int tempPos = curPos; tempPos > newPos; tempPos -= 1)  // goes from curPos (degrees) to newPos (degrees) 
      {                                  // in steps of 1 degree 
        servo.write(tempPos);              // tell servo to go to position in variable 'tempPos' 
...
void processInput(int inputData) {
  ...
  if( (waitForData > 0) && (inputData < 128) ) {  
    ...
    if( (waitForData==0) && executeMultiByteCommand ) { // got the whole message
      ...
      case SERVO_MESSAGE:
                setServoPos(multiByteChannel, 
                (storedInputData[0] << 7) + storedInputData[1]); // (pin#, pos)
        break;
      case ANALOG_MESSAGE:
...
void setup() {
   ...
   * change. */
   ...
   int start = 9;
   int n = numServos + start;
   for (i=start; i < n; i++)
   {
    setPinMode(i, OUTPUT);
    servo.attach(i);
    servo.write(servoInputs[i-start]);
    delay(500);  // allow time to reach the new position 
   }
   servo.detach();
}
