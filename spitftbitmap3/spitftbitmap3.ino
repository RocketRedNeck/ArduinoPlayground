...
void loop() 
{
  ...
  if (xPos >= (xScreenSize - (landscape?bmpHeight:bmpWidth)))
  {
    xDirection *= -1;
  }
  else if (xPos <= 0)
  ...
  if (yPos >= (yScreenSize - (landscape?bmpWidth:bmpHeight)))
  {
    yDirection *= -1;
  }
  else if (yPos <= 0)
  ...
  {
    // x, y, w, h
    VCLEAR(lastX , lastY, lastDirX, bmpHeight, ILI9341_BLACK);
  }
  else if (lastDirX < 0)
  {
    VCLEAR(lastX + bmpWidth + lastDirX, lastY, -lastDirX, bmpHeight, ILI9341_BLACK);
  }
  
  if (lastDirY > 0)
  {
    HCLEAR(lastX, lastY, bmpWidth, lastDirY, ILI9341_BLACK);
  }
  else if (lastDirY < 0)
  {
    HCLEAR(lastX, lastY + bmpHeight + lastDirY, bmpWidth, -lastDirY, ILI9341_BLACK);
  }
  
  bmpDraw(ICON_NAME, xPos, yPos);
