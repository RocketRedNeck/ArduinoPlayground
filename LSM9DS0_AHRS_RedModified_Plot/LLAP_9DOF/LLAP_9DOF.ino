...
void processSchedule(void)
{
  ...
  if (cycle)
  {
    if (aboutNow_ms >= schedTime_ms)
    {
      ...
      reply = "AX";
      reply += String(lsm.accelData.x * aLsb, 5);
      LLAP.sendMessage(reply);
      reply = "AY";
      reply += String(lsm.accelData.y * aLsb, 5);
      LLAP.sendMessage(reply);
      reply = "AZ";
      reply += String(lsm.accelData.z * aLsb, 5);
      LLAP.sendMessage(reply);

      reply = "GX";
      reply += String(lsm.gyroData.x * gLsb, 5);
      LLAP.sendMessage(reply);
      reply = "GY"; 
      reply += String(lsm.gyroData.y * gLsb, 5);
      LLAP.sendMessage(reply);
      reply = "GZ";
      reply += String(lsm.gyroData.z * gLsb, 5);
      LLAP.sendMessage(reply);

       reply = "MX";
      reply += String(lsm.magData.x * mLsb, 5);
      LLAP.sendMessage(reply);
      reply = "MY"; 
      reply += String(lsm.magData.y * mLsb, 5);
      LLAP.sendMessage(reply);
      reply = "MZ";
      reply += String(lsm.magData.z * mLsb, 5);
      LLAP.sendMessage(reply);
